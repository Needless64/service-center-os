...
export async function handleIncomingMessage(phone: string, text: string, interactiveId?: string) {
  ...
  // Interactive button/list replies handled first
  if (interactiveId) {
    await handleInteractiveReply(phone, interactiveId)
    return
  }

  // State machine: text input during booking flow
  const state = session.state
  if (
    state === 'COLLECTING_VEHICLE_NUMBER' ||
    state === 'COLLECTING_VEHICLE_MODEL' ||
  ...
  switch (intent) {
    ...
    case 'CHECK_STATUS':
      ...
      break
    case 'CANCEL_BOOKING':
      await handleCancelRequest(phone)
      break
    case 'CONFIRM_YES':
...
// ─── Welcome menu with tappable buttons ──────────────────────────────────────

async function sendWelcome(phone: string) {
  await sendButtons(
    phone,
    `👋 Welcome! I'm your *Service Center assistant*.\n\nWhat would you like to do?`,
    [
