...
import { generateTimes, normalizePhone } from "../utils"; // Import generateTimes from utils
...
const BOOKING_LEAD_MINUTES = 60; // 1 hour lead time (IST)
...
// Removed local generateTimes function
...
export async function getAvailableDays(daysAhead = 7): Promise<DaySlots[]> {
  ...
  const times = generateTimes(
    ...
    branch.slotDurationMinutes, // Pass slotDurationMinutes from branch
  );
  ...
  for (const dateStr of dateStrings) {
    ...
    const dayLabel =
      dateStr === todayStr
        ? `Today · ${format(dateObj, "EEE")} ${fullDate}`.slice(0, 24)
        : dateStr === tomorrowStr
          ? `Tmr · ${format(dateObj, "EEE")} ${fullDate}`.slice(0, 24)
