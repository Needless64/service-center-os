...
export async function handleServiceTypeSelection(
  phone: string,
  ...
  if (session.isReturningCustomer && session.existingCustomerId) {
    ...
    if (customer?.vehicles?.length) {
      await updateSession(phone, {
        serviceType,
        state: "COLLECTING_VEHICLE_NUMBER",
      });
      await sendButtons(
        phone,
  ...
  await sendText(
    phone,
    `*${formatServiceType(serviceType)}* ${t(lang, "booking.vehicle.selected", "")}\n${t(lang, "booking.vehicle.number_prompt")}`,
  );
}
  ...
  switch (session.state) {
    case "COLLECTING_VEHICLE_NUMBER": {
      ...
      await updateSession(phone, {
        ...
        vehicleModel: "",
        state: session.isReturningCustomer
          ? "SELECTING_SLOT_DATE"
...
async function showSlotSelection(phone: string) {
  ...
  if (days.length === 0) {
    await sendText(
      phone,
      `😔 No slots available in the coming days.\n\nPlease call us directly to book.`,
    );
...
export async function handleDaySelection(phone: string, dateStr: string) {
  ...
  if (!day || day.slots.length === 0) {
    await sendText(
      phone,
      `No slots available for that day. Please choose another.`,
    );
    await showSlotSelection(phone);
    return;
  }
  ...
  await sendList(phone, `*${day.dayLabel}*`, "Pick Time", [
    {
      title:
        day.dayLabel.length > 24 ? day.dayLabel.slice(0, 24) : day.dayLabel,
      rows,
    },
  ]);
}
...
export async function confirmBooking(phone: string) {
  ...
  try {
    ...
    if (!customerId) {
      ...
      const vehicleExists = existing?.vehicles?.some(
        (v) => v.vehicleNumber === session.vehicleNumber,
      );
      if (!vehicleExists)
    ...
    const timeFormatted = format(
      new Date(`2000-01-01T${session.selectedTime}`),
      "h:mm a",
    );
