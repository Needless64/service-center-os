...
const customerByPhoneQuery = defineQuery(
  `*[_type == "customer" && phoneNumber == $phone][0]`,
);

export async function getCustomerByPhone(
  phone: string,
): Promise<SanityCustomer | null> {
  return sanityClient.fetch(customerByPhoneQuery, {
...
const bookingByIdQuery =
  defineQuery(`*[_type == "booking" && bookingId == $bookingId][0]{
  ...,
...
export async function getBookingById(
  bookingId: string,
): Promise<SanityBooking | null> {
  return readClient.fetch(bookingByIdQuery, { bookingId });
}

const bookingsByDateQuery =
  defineQuery(`*[_type == "booking" && scheduledDate == $date] | order(scheduledTime asc){
  ...,
  customer->{ _id, name, phoneNumber }
}`);
...
export async function getBookingsByDate(
  date: string,
): Promise<SanityBooking[]> {
  return sanityClient.fetch(bookingsByDateQuery, { date });
}

const activeBookingForCustomerQuery = defineQuery(
  `*[_type == "booking" && customer._ref == $customerId && status in ["booked","received"]] | order(scheduledDate desc)[0]`,
);
...
const allActiveBookingsForCustomerQuery = defineQuery(
  `*[_type == "booking" && customer._ref == $customerId && status in ["booked","received"]] | order(scheduledDate asc)`,
);

export async function getAllActiveBookingsForCustomer(
  customerId: string,
): Promise<SanityBooking[]> {
  return sanityClient.fetch(allActiveBookingsForCustomerQuery, { customerId });
}

const upcomingBookingsForRemindersQuery =
  defineQuery(`*[_type == "booking" && status == "booked" && scheduledDate >= $today && reminderSent24h != true && reminderSent3h != true && reminderSent30m != true]{
  ...,
  customer->{ _id, name, phoneNumber }
}`);
...
const latestActiveBookingQuery =
  defineQuery(`*[_type == "booking" && customer._ref == $customerId && status in ["booked","received"] && scheduledDate >= $today] | order(scheduledDate asc, scheduledTime asc)[0]{
  ...,
  customer->{ _id, name, phoneNumber }
}`);
...
const availableSlotsQuery = defineQuery(
  `*[_type == "slot" && date in $dates && isBlocked != true && currentBookings < capacity] | order(date asc, time asc)`,
);
...
export async function getSlot(
  date: string,
  time: string,
): Promise<SanitySlot | null> {
  return readClient.fetch(slotQuery, { date, time });
}

// ─── Branch queries ──────────────────────────────────────────────────────────

const defaultBranchQuery = defineQuery(
  `*[_type == "branch" && isActive == true][0]`,
);
...
const allBookingsForSheetsQuery =
  defineQuery(`*[_type == "booking"] | order(scheduledDate desc){
